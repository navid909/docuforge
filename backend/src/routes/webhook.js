...
export const webhookRoutes = FastifyPluginAsyncTypebox({
  async fastify({ reply, log }) {
    ...
    fastify.post('/stripe', {
      ...
      handler: async (request, reply) => {
        if (!stripeSecret) {
          log.warn('STRIPE_WEBHOOK_SECRET not configured');
        ...
        try {
          ...
          log.error({ err, signature }, 'Stripe webhook signature verification failed');
          return reply.code(400).send({ error: `Webhook signature verification failed: ${err.message}` });
        }

        log.info({ event: event.type }, 'Stripe webhook received');
        try {
          await handleStripeEvent(event, prisma, log);
          return reply.send({ received: true });
        } catch (err) {
          log.error({ err, event: event.type }, 'Webhook handler error');
    ...
    async function handleStripeEvent(event, prisma, log) {
      if (event.type === 'checkout.session.completed') {
        ...
        const subscription = await prisma.prisma.subscription.findFirst({
        ...
        const subs = await prisma.prisma.subscription.findFirst({
        ...
        if (subs) {
          ...
          log.info({ userId: subs.userId }, 'Premium revoked due to subscription cancellation');
        }
        ...
        if (subs) {
          ...
          const user = await prisma.prisma.user.findUnique({ where: { id: subs.userId } });
          if (user) {
            if (status === 'active' && user.isPremium === false) {
